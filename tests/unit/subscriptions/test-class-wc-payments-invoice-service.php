...
class WC_Payments_Invoice_Service_Test extends WP_UnitTestCase {
	...
	/**
	 * Mock WC Subscription object.
	 *
	 * @var WC_Subscription
	 */
	private $mock_subscription;

	/**
	 * Mock WC Order object.
	 *
	 * @var WC_Order
	 */
	private $mock_order;
	...
	public function setUp() {
		...
		$this->mock_subscription = new WC_Subscription();

		$this->mock_subscription->set_parent( $this->mock_order );

		$this->mock_api_client      = $this->createMock( WC_Payments_API_Client::class );
		$this->mock_invoice_service = new WC_Payments_Invoice_Service( $this->mock_api_client, null );
	}
	...
	public function test_mark_pending_invoice_for_subscription() {
		...
		$this->mock_invoice_service->mark_pending_invoice_for_subscription( $this->mock_subscription, $invoice_id );
		$this->assertEquals( $invoice_id, $this->mock_subscription->get_meta( self::PENDING_INVOICE_ID_KEY, true ) );
	}
	...
	public function test_mark_pending_invoice_paid_for_subscription() {
		$this->mock_invoice_service->mark_pending_invoice_paid_for_subscription( $this->mock_subscription );
		$this->assertEquals( '', $this->mock_subscription->get_meta( self::PENDING_INVOICE_ID_KEY, true ) );
	}
	...
	public function test_delete_invoice_items() {
		...
		$this->mock_invoice_service->delete_invoice_items( $invoice_ids );
	}
	...
	public function test_get_pending_invoice_id() {
		$this->assertEquals( '', $this->mock_invoice_service->get_pending_invoice_id( $this->mock_subscription ) );

		$this->mock_subscription->update_meta_data( self::PENDING_INVOICE_ID_KEY, 'in_test123' );
		$this->assertEquals( 'in_test123', $this->mock_invoice_service->get_pending_invoice_id( $this->mock_subscription ) );
	}
	...
	public function test_set_pending_invoice_id() {
		PHPUnit_Utils::call_method(
			...
			[ $this->mock_subscription, 'in_test_abc' ]
		);
		$this->assertEquals( 'in_test_abc', $this->mock_subscription->get_meta( self::PENDING_INVOICE_ID_KEY, true ) );
	}
	...
	public function test_get_order_invoice_id() {
		$this->assertEquals( '', $this->mock_invoice_service->get_order_invoice_id( $this->mock_order ) );

		$this->mock_order->update_meta_data( self::ORDER_INVOICE_ID_KEY, 'in_foo' );
		$this->assertEquals( 'in_foo', $this->mock_invoice_service->get_order_invoice_id( $this->mock_order ) );
	}
	...
	public function test_set_order_invoice_id() {
		$this->mock_invoice_service->set_order_invoice_id( $this->mock_order, 'in_bar' );
		$this->assertEquals( 'in_bar', $this->mock_order->get_meta( self::ORDER_INVOICE_ID_KEY, true ) );
	}
	...
	public function test_get_order_id_by_invoice_id() {
		$this->mock_order->update_meta_data( self::ORDER_INVOICE_ID_KEY, 'in_invoice123' );
		$this->mock_order->save();

		$this->assertEquals( $this->mock_order->get_id(), $this->mock_invoice_service->get_order_id_by_invoice_id( 'in_invoice123' ) );
		$this->assertEquals( 0, $this->mock_invoice_service->get_order_id_by_invoice_id( 'invalid_invoice_id' ) );
	}
	...
	public function test_prepare_invoice_item_data() {
		$this->mock_order->set_discount_total( 20 );
	...
	public function test_create_invoice_items_for_subscription() {
		...
		$this->mock_invoice_service->create_invoice_items_for_subscription( $this->mock_subscription, $wcpay_customer_id, $wcpay_subscription_id );
	}
	...
	public function test_maybe_record_first_invoice_payment() {
		...
		$this->mock_order->update_meta_data( self::ORDER_INVOICE_ID_KEY, $invoice_id );
		$this->mock_order->save();
		...
		$this->mock_invoice_service->maybe_record_first_invoice_payment( $this->mock_order->get_id(), 'pending', 'processing' );
	}
